...
const ScheduleLoader = {
    ...
    async loadScheduleFromJSON(sport, category, team) {
        try {
            const apiBaseURL = window.API?.baseURL || 'http://localhost:8000';
            const url = `${apiBaseURL}/api/schedules/${sport}/${category}/${team}`;
...
// Export for use in other files - execute immediately
(function() {
    'use strict';
    try {
        if (typeof window !== 'undefined') {
            window.ScheduleLoader = ScheduleLoader;
            console.log('[DEBUG] ScheduleLoader exported to window', { 
                hasLoadScheduleFromJSON: typeof ScheduleLoader.loadScheduleFromJSON === 'function',
                hasConvertToMatches: typeof ScheduleLoader.convertToMatches === 'function',
                methods: Object.keys(ScheduleLoader),
                timestamp: new Date().toISOString()
            });
        } else {
            console.error('[DEBUG] window is undefined, cannot export ScheduleLoader');
        }
    } catch (error) {
        console.error('[DEBUG] Error exporting ScheduleLoader:', error);
    }
})();
